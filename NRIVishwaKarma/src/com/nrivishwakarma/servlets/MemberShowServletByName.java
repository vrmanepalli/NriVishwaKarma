...
public class MemberShowServletByName extends HttpServlet{
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("data of the name from the front end "+sd.searchBox);
		try {
			DBMembers dbm = new DBMembers();
			System.out.println("before calling the function");
			ls = dbm.memberFunctionByName(sd.searchBox);
		} catch (Exception e) {
			status=false;
			e.printStackTrace();
		}
